class Admin::PositionsController < Admin::ApplicationController
  ...
  def create
    ...
    if @position.errors.messages.any?
      render json: { validate: true, data: errors_data(@position) }, status: 422
    else
  ...
  def position_params
    params.require(:position).permit(:title, :duties, :description, :landing_title,
                                     :slug, :price_group_id, :warranty_period)
  end
